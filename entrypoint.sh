...
# Find the exact PHP-FPM binary name dynamically (handles php-fpm, php-fpm8.1, php-fpm8.2, etc.)
...
# Override socket settings in all pool files to use TCP port 9000
...
# Run the binary with an explicit flag to force it to listen on 9000 as a daemon
$FPM_BIN -d "listen=127.0.0.1:9000" -D || $FPM_BIN -D
